...
def test_default_runtime_configs_are_packaged() -> None:
    ...
    assert (CONFIG_DIR / "rack_box_poses_small_middle.json").is_file()
    ...
    assert (PACKAGE_CONFIG_DIR / "rack_box_poses_small_middle.json").is_file()
...
def test_deployment_and_wheel_fallback_configs_are_synchronized() -> None:
    ...
    for name in (
        "workcell_layout.json",
        "rack_box_poses.json",
        "rack_box_poses_small_middle.json",
        "grippers.json",
    ):

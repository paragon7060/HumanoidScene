...
def test_task_posture_target_and_direct_bias_share_the_bounded_servo():
    joints = torch.zeros(1, 7)
    posture = joints.clone()
    posture[0, 5] = .4
    jac = torch.zeros(1, 6, 7)
    limits = torch.tensor([-1., 1.]).expand(1, 7, 2)
    ...
    assert velocity[0, 5] == pytest.approx(SMOOTH.max_acceleration / 60)
    assert command[0, 5] > 0
    torch.testing.assert_close(velocity[0, :5], torch.zeros(5))
    assert velocity[0, 6] == 0

...
def test_calibrated_cycle_matches_independently_recorded_tip_widths():
    settings = load_gripper_settings("leju-twofinger")
    geometry = TwoFingerGeometry()
    mapper = DirectionalGripperMapping(settings.sides["left"].position_mapping, 1, "cpu")
    opened = settings.command_for("left", settings.open_command)["l_f_bar_1_joint"]
    for percent, width in zip([0,25,50,75,100,75,50,25,0], [90,78,51,26,0,18,40,68,90]):
        fraction = float(mapper.process(torch.tensor([[1-percent/50]]))[0,0])
        assert geometry.gap_m(opened * (1-fraction))*1000 == pytest.approx(width, abs=.03)
    assert geometry.gap_m(-.25)*1000 == pytest.approx(67.3369, abs=.001)
    assert settings.command_for("right", settings.open_command)["r_f_bar_1_joint"] == opened
    assert settings.sides["right"].position_mapping == settings.sides["left"].position_mapping
    assert load_gripper_settings("s200062_integrated").command_for("right", settings.open_command)["r_f_bar_1_joint"] == -.25
...
def test_lookup_matches_width_interpolation_between_measured_points():
    settings = load_gripper_settings("leju-twofinger")
    mapper = DirectionalGripperMapping(settings.sides["left"].position_mapping, 1, "cpu")
    geometry = TwoFingerGeometry()
    opened = settings.command_for("left", settings.open_command)["l_f_bar_1_joint"]
    for command, widths in [(np.linspace(0,100,201), [90,78,51,26,0]),
                            (np.linspace(100,0,201), [90,68,40,18,0])]:
        for percent in command:
            fraction = float(mapper.process(torch.tensor([[1-percent/50]]))[0,0])
            expected = np.interp(percent, [0,25,50,75,100], widths)
            assert geometry.gap_m(opened*(1-fraction))*1000 == pytest.approx(expected, abs=.03)


@pytest.mark.parametrize("side", ["left", "right"])
def test_repeated_75_reference_is_shared_by_both_approaches_without_new_branch(side):
    settings = load_gripper_settings("leju-twofinger")
    geometry = TwoFingerGeometry(side)
    mapper = DirectionalGripperMapping(settings.sides[side].position_mapping, 2, "cpu")
    opened = settings.command_for(side, settings.open_command)[f"{side[0]}_f_bar_1_joint"]
    for commands, expected in [([0,100],[90,0]), ([25,50],[78,40]),
                               ([50,55],[51,40]), ([75,60],[26,40]),
                               ([100,75],[0,26])]:
        fractions = mapper.process(1-torch.tensor(commands).reshape(2,1)/50)
        for fraction, width in zip(fractions[:,0], expected):
            assert geometry.gap_m(opened*(1-float(fraction)))*1000 == pytest.approx(width, abs=.03)

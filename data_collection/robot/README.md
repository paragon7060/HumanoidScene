...
- 좌우 `zarm_[lr]7_end_effector`는 URDF에서 마지막 손목 링크로부터 z=-0.17m. 기존 teleop는 이 링크를 직접 사용한다. `rl/tasks/specs.py`의 추가 tool offset z=-0.12m와 혼용하지 않는다. 이번 YAML은 teleop 기준 추가 offset=0이며 physical TCP는 미검증이다. Task1 웹 확인용 pregrasp는 box 좌우 flap의 상단 grasp 쌍을 먼저 계산하고, 각 점에서 robot-base +Z 방향으로 설정 높이만큼 올린 위치를 사용한다. 좌우 바깥 법선 방향으로 벌리는 점은 pregrasp로 사용하지 않는다.

...
- S200062 two-finger의 좌우 `zarm_[lr]7_end_effector`는 닫힌 손가락 메시의 접촉대 중심에 맞춰 마지막 손목 링크로부터 z=-0.22m다. `twofinger_base` 기준으로는 z=-0.179m이며, 완전 닫힘 메시의 실제 접촉 구간(z=-0.175~-0.188m) 안이다. `rl/tasks/specs.py`의 추가 tool offset z=-0.12m와 혼용하지 않는다. Task1 웹 확인용 pregrasp는 box 좌우 flap의 상단 grasp 쌍을 먼저 계산하고, 각 점에서 robot-base +Z 방향으로 설정 높이만큼 올린 위치를 사용한다. 좌우 바깥 법선 방향으로 벌리는 점은 pregrasp로 사용하지 않는다.

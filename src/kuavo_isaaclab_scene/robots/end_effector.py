...
ORIGINAL_EEF_FRAMES = ("zarm_l7_end_effector", "zarm_r7_end_effector")
CENTER_FRAME_NAME = "endeffector_center"
CENTER_TOOL_FRAMES = {
    "left": "zarm_l7_endeffector_center",
    "right": "zarm_r7_endeffector_center",
}


def center_position_from_original_pose(position, rotation, offset):
    """Return the calibrated closed TCP position for one original EEF pose."""
    position = np.asarray(position, dtype=float)
    rotation = np.asarray(rotation, dtype=float)
    offset = np.asarray(offset, dtype=float)
    if position.shape != (3,) or rotation.shape != (3, 3) or offset.shape != (3,):
        raise ValueError("center transform requires xyz, 3x3 rotation, and xyz offset")
    return position + rotation @ offset


def original_position_for_center_target(position, rotation, offset):
    """Map an ``endeffector_center`` target to the original EEF target position."""
    position = np.asarray(position, dtype=float)
    rotation = np.asarray(rotation, dtype=float)
    offset = np.asarray(offset, dtype=float)
    if position.shape != (3,) or rotation.shape != (3, 3) or offset.shape != (3,):
        raise ValueError("center target transform requires xyz, 3x3 rotation, and xyz offset")
    return position - rotation @ offset


def urdf_with_center_frames(urdf_text, offsets=None):
    """Add nonphysical fixed planner frames for the calibrated closed TCPs.

    URDF link names are global, so the two USD children named
    ``endeffector_center`` use side-qualified names in this in-memory planner
    description. The source URDF and its original EEF frames remain unchanged.
    """
    root = ET.fromstring(urdf_text)
    definition = calibration_definition() if offsets is None else None
    offsets = offsets or (definition["closed_offsets"] if definition else None)
    if offsets is None:
        raise ValueError("No calibrated endeffector_center definition is available")
    link_names = {link.get("name") for link in root.findall("link")}
    for side, letter in (("left", "l"), ("right", "r")):
        original = f"zarm_{letter}7_end_effector"
        center = CENTER_TOOL_FRAMES[side]
        if original not in link_names:
            raise ValueError(f"Missing original EEF link: {original}")
        offset = np.asarray(offsets[side], dtype=float)
        if offset.shape != (3,) or not np.isfinite(offset).all():
            raise ValueError(f"Invalid {side} endeffector_center offset")
        if center in link_names:
            raise ValueError(f"Planner center frame already exists: {center}")
        ET.SubElement(root, "link", {"name": center})
        joint = ET.SubElement(
            root, "joint", {"name": f"{center}_joint", "type": "fixed"}
        )
        ET.SubElement(
            joint,
            "origin",
            {"xyz": " ".join(format(value, ".17g") for value in offset), "rpy": "0 0 0"},
        )
        ET.SubElement(joint, "parent", {"link": original})
        ET.SubElement(joint, "child", {"link": center})
        link_names.add(center)

...
def author_closed_linkages(stage, *, sides: str = "lr", actuator: dict | None = None) -> None:
    ...
    driver_stiffness = 100.0 if actuator is None else float(actuator["stiffness"])
    driver_damping = 10.0 if actuator is None else float(actuator["damping"])
    driver_max_force = 5.0 if actuator is None else float(actuator["effort_limit_sim"])
    if any(not math.isfinite(value) or value < 0 for value in
           (driver_stiffness, driver_damping, driver_max_force)):
        raise ValueError("Claw actuator stiffness, damping and effort limit must be finite and non-negative")
    for side in _linkage_sides(sides):
        for jaw in "fb":
            ...
            for index in (1, 3, 4):
                ...
                drive.CreateStiffnessAttr(driver_stiffness * math.pi/180 if index == 1 else 0.0)
                drive.CreateDampingAttr(driver_damping * math.pi/180 if index == 1 else 0.0)
                drive.CreateMaxForceAttr(driver_max_force if index == 1 else 5.0)

...
def per_body_forces(
    ...
    """Return each mapped body's rack and non-rack contact force magnitude.

    The rack column is the largest filtered rack/roller contact on that body and
    the non-rack column is its remaining net force, so operator diagnostics and
    the aggregate safety term always report the same contact.
    """
    ...
    for force, index in zip(rack_force_matrices, body_indices, strict=True):
        ...
        rack_columns.append(force[:, 0].norm(dim=-1).amax(dim=-1))
        non_rack_columns.append((net_forces_w[:, index] - force[:, 0].sum(dim=1)).norm(dim=-1))
    return torch.stack(rack_columns, dim=-1), torch.stack(non_rack_columns, dim=-1)
...
def maximum_non_rack_force(
    ...
    bodies_without_sensors = [
        index for index in range(net_forces_w.shape[1]) if index not in set(body_indices)
    ]
    non_rack = per_body_forces(net_forces_w, rack_force_matrices, body_indices)[1]
    unfiltered = net_forces_w[:, bodies_without_sensors].norm(dim=-1)
    return torch.cat((non_rack, unfiltered), dim=-1).amax(dim=-1)

...
[Quest 빠른 시작](QUEST3_QUICKSTART.md)을 먼저 따른다.
...
## 선택 기능: 오른쪽 그립 길게 눌러 맨손으로 전환

**기존 컨트롤러 실행은 그대로다.** 아래 수집기 명령에 `--hand-switch`를 추가한 실행에서만
전환 기능과 작은 우측 상단 상태 표시가 켜진다. 기존 A/B/X/Y·검지 트리거·왼쪽 그립·스틱 매핑은 유지한다.
`--hand-switch`는 `--controller-mapping scaled` 또는 `absolute`에서만 지원한다.
손 모드는 항상 1.1배 기본 위치 매핑과 보정 시점 대비 1:1 회전을 사용한다.

1. Quest의 손 추적과 손/컨트롤러 자동 전환을 활성화하고 브라우저 손 추적 권한을 허용한다.
2. 컨트롤러로 시점·팔을 준비한 뒤 **오른쪽 아래 그립(중지로 누르는 squeeze)을 1.2초 이상** 누른다.
   오른쪽 위 검지 트리거가 아니다. 이전 녹화가 있으면 `input_mode_switch`로 종료하고 보존한다.
3. 상태 표시의 **3초 카운트다운** 동안 컨트롤러를 내려놓고 양손을 편하게 보인다.
   로봇 팔·gripper·base 명령은 유지한다. 시간이 지나도 양손이 인식되지 않으면 계속 대기한다.
4. 실제 맨손의 손목·엄지끝·검지끝과 head가 0.5초 연속 유효하면 위치·방향 기준을 다시 잡고
   **새 파일로 손 모드 녹화를 시작**한다. 전환 준비 구간은 새 파일에 넣지 않는다.
   반쯤 쥔 물체를 놓치지 않도록 gripper는 전환 전 상태를 유지하고, 손 pinch가 그 상태와 한 번
   일치한 다음부터 개폐를 따른다. 처음에는 물체를 잡지 않은 상태에서 전환을 시험한다.

| 맨손 조작 | 동작 |
| --- | --- |
| 엄지 + 검지 pinch | 같은 쪽 gripper 닫기. 5.5cm 이하 닫힘 / 7cm 이상 열림으로 떨림 방지 |
| 왼손 엄지 + **중지**, 검지를 펴고 1초 유지 | A와 같은 따라오기 정지/재개·위치/방향 재보정 |
| 오른손 엄지 + **중지**, 검지를 펴고 1초 유지 | B와 같은 녹화 시작/중지. 시작은 제스처를 푼 뒤 실행 |
| PC C / M / R | 시점 보정 / 성공 종료 / 장면 초기화 |
| 컨트롤러를 다시 잡고 오른쪽 아래 그립 1.2초 | 컨트롤러 복귀 요청. 3초 후 양쪽 컨트롤러가 유효하고 그립을 놓으면 **정지 상태**로 복귀 |

중지 제스처는 엄지-중지 2.5cm 이하, 엄지-검지 7cm 이상일 때만 후보가 된다. 제스처를 하는 손의
팔·gripper 목표를 유지한다. 중지 제스처가 잘 안 되면 PC T/P로 동일 기능을 사용할 수 있다.
**손 모드에서도 A에 해당하는 정지는 녹화 파일을 끝낸다. 같은 파일을 유지하는 clutch가 아니다.**
전환 대기는 PC P로 취소하거나 오른쪽 그립을 놓았다가 다시 길게 눌러 취소한다.

손 모드에서는 **베이스 이동·회전과 몸통 높이를 고정**한다. 양손을 돌려도 베이스를 돌리지 않는다.
그 조작은 이번 비교 실험에 넣지 않았다. 추적이 잠깐 사라지면 해당 팔과 gripper를 유지하며,
2초 이상 양손/head 추적 조건이 충족되지 않으면 따라오기와 현재 녹화를 종료한다.
...
이 표시는 아직 학습 시 자동 제외 필터로 연결하지 않았다.

검증 명령(실제 사용자 dataset에 쓰지 않음):

```bash
PYTHONPATH=src KUAVO_ROBOT_MODEL=s200062 python scripts/verify_quest_controls.py
# 실제 collector/RTX/HDF5 + 합성 입력. 헤드셋과 서버는 필요 없음.
PYTHONPATH=src KUAVO_ROBOT_MODEL=s200062 python scripts/verify_quest_hand_switch.py
# Runtime과 Quest 연결이 준비됐을 때만 XR 상태 표시까지 검사
PYTHONPATH=src KUAVO_ROBOT_MODEL=s200062 python scripts/verify_quest_hand_switch.py --xr-display
```

합성 검증은 실제 손의 가림·떨림·제스처 인식률이나 Quest 내 UI 가독성을 보장하지 않는다.
두 모드는 같은 30Hz·해상도·환경에서 중간 높이 상자 파지를 비교한다.

2026-09-01 검증: 단위·소켓 112개 통과. 기존 컨트롤러 물리 검증에서 전방 pose 오차 약 0.91mm,
방향 유지 +10cm 이동 오차 약 1.21mm로 이전 결과와 같았고 베이스·몸통·head·gripper도 통과했다.
합성 입력을 실제 수집기에 넣어 컨트롤러 2개/손 2개의 별도 HDF5, 24-D action, head RGB,
손 모드 베이스 고정, 2초 추적 손실 종료, 컨트롤러 정지 복귀를 확인했다. 상태 위젯의 Kit API
생성·텍스트 갱신은 확인했지만 실제 Quest 화면 표시는 미검증이다.
...
손목 카메라를 별도로 켠 실행에서만 **Y**(**H**)로 head/양 손목 패널 표시·숨김을 제어한다.
...
맨손만 시작하려면 `--input-mode hands`, 실행 중 선택적으로 전환하려면 `--hand-switch`를 사용한다.
현재 모드는 `[INFO] Arm input mode`
...
손목 카메라를 켠 실행에서 패널이 검으면 `[CAMERA] Left wrist`,
`[CAMERA] Head`, `[CAMERA] Right wrist` RGB 로그의 `max`가 0인지 먼저 확인한다.
센서 영상이 정상인데 패널만 검다면 XR UI 경로 문제다. 현재 위젯은 Frame 안에
이미지 레이아웃을 직접 배치하며, 첫 유효 프레임 전의 검은 자리표시는 숨긴다.
카메라 패널은 눈앞 0.35 m에 표시한다. 더 조절하려면 `--xr-overlay-distance`를 쓰되

...
| 몸통 전후 이동(위=앞, 아래=뒤) | **오른쪽 아래 그립(squeeze) + 오른쪽 스틱 상하** | — |
| 허리 yaw 회전 | **오른쪽 아래 그립(squeeze) + 오른쪽 스틱 좌우** | — |
| 누르는 동안 자유 시점, 놓으면 head 복귀 | 왼쪽 아래 그립 트리거 | — |
| 놓으면 open, 당기면 close | 양쪽 위 검지 트리거 | — |

여기서 아래 그립 트리거는 중지로 쥐는 `squeeze`이고, 위 검지 트리거는 gripper
개폐 입력이다. 오른쪽 squeeze를 누른 동안에만 오른쪽 스틱이 몸통 전후·허리 yaw
모드로 바뀐다. 왼쪽 스틱의 베이스 이동은 계속 사용할 수 있다. 오른쪽 squeeze를
놓으면 오른쪽 스틱은 다시 베이스 회전·몸통 승강으로 돌아가며, 바꾼 몸통 전후 위치와
허리 각도는 마지막 목표로 유지한다.
...
오른쪽 squeeze와 스틱을 함께 움직이면 해당 squeeze는 성공 종료나 맨손 전환의
길게 누르기로 처리되지 않는다. 몸통 조작을 끝낸 뒤 squeeze를 완전히 놓아야 long-press가
다시 활성화된다. 반대로 스틱을 중앙에 둔 채 squeeze만 먼저 오래 누르면 기존 long-press가
실행될 수 있으므로 몸통 조작은 squeeze와 스틱을 함께 시작한다.
...
검증용 장면도 최종 `SmallBox_atlas.usda`를 unit scale로 사용한다. 실측 본체 크기는
0.266×0.185×0.130m(W×D×H), flap 길이는 0.100m이며 원본 collision과 질량을 유지한다.

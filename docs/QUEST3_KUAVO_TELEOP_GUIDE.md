...
1.5배 확대하여 같은 쪽 로봇 손끝의 목표로 사용한다. 검지 트리거는 절반 이상 당기면 닫고 놓으면 연다.
...
기본 `--controller-mapping scaled --position-gain 1.5`는 사람과 로봇의 팔 길이 차이를 보완한다.
정면을 보고 X로 시점을 맞춘 다음, 양손을 몸 앞에 편하게 두고 A를 누른다. 각 손의 첫 유효한
컨트롤러 위치와 현재 로봇 손끝을 연결해 기준점으로 삼는다. 그 위치에서 손을 앞으로/아래로
20cm 옮기면 손끝 목표는 30cm 이동한다. 방향에는 위치 gain을 곱하지 않는다.

A로 정지한 뒤 손을 편한 위치로 옮기고 A를 다시 누르면 현재 로봇 손끝부터 새로운 기준으로
이어갈 수 있다. 추적이 잠깐 끊기는 것만으로 기준을 바꾸지는 않는다. 몸통 좌표계에서 변위를
계산하므로 베이스 이동·회전이나 몸통 높이 변경은 배율만큼 과장되지 않는다.
`--position-gain 1.8`처럼 1.0~3.0 범위에서 조절할 수 있지만, gain이 크면 손 떨림도 커진다.
현재 1.5부터 시작하며 팔 속도·가속도 제한과 필터는 유지한다.
...
기본 확대 매핑은 `scaled`, gain은 1.5다. `X`로 시점을 맞추고 손을 편하게 든 상태에서 `A`를 누른다.
팔을 더 뻗을 공간이 부족하면 `A` 정지 → 손을 편하게 이동 → `A` 재개로 기준을 옮긴다.
예를 들어 아래로 20cm 움직이면 로봇 목표는 30cm 내려간다. 실제 로봇의 관절 한계는 그대로다.

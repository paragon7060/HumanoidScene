...
새 하위 task별 RL 작업은 `rl/envs/`, `rl/scenes/`, `rl/tasks/`, `rl/managers/`, `rl/mdp/`에서 시작한다.
RL은 `rl/scenes/scene_cfg.py`의 독립적인 scene을 사용하며 일반 manager scene을
import하거나 상속하지 않는다. 공장/이동 로봇/장식용 네트워크 conveyor는 생성하지 않는다.
병렬 설정과 수정 대응표는 [RL 전용 병렬 환경](RL_PARALLEL_ENVS.md)을 참고한다.

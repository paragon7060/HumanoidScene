...
| 오른쪽 아래 그립 + 오른쪽 stick 위/아래 | 몸통 전후 이동 (위=앞 / 아래=뒤) |
| 오른쪽 아래 그립 + 오른쪽 stick 좌/우 | 허리 좌우 회전 |
| 양쪽 검지 trigger | 해당 gripper 닫기/열기 |

베이스 최고 속도는 0.15 m/s, 회전은 0.45 rad/s, 허리 승강은 0.10 m/s이며
출발·스틱 해제·방향 전환에 공통 가속도 제한을 적용한다.
[베이스·허리 속도 설정](QUEST_BODY_CONTROL.md)에서 감속 거리와 안전 정지 차이를 확인한다.
처음 몸통 높이는 하한이므로 먼저 올려야 내릴 수 있다.

...
기존 단위 템플릿 좌표를 그대로 미터로 간주하지 않는다. S200062 integrated gripper는
`configs/grasp_reference_points.json`과 실제 닫힘 명령의 FK로 계산한 고정 TCP를
접근 기준점으로 사용한다. 실제 움직이는 손가락의 보정점은 접촉 방향 판정에 사용한다.
접촉 성공은 PhysX 센서로 판정한다. `--grasp-debug-vis`는 env 0에서 보정된 손가락점(청록),
TCP(노랑), 최근접 flap 목표(초록), 움직이는 두 손가락점의 중점(빨강)을 표시한다.
...
신호를 제공한다. 접근 reward는 `4 * (exp(-12 * 현재 거리) - exp(-12 * 이전 거리))`이고 방향 정렬로 곱하지 않는다.
정지하면 0, 접근하면 양수, 멀어지면 음수이며 파지 획득·유지·상실 전환 스텝에서는 0이다.
...
대신 `rack_roller_runtime.usda`가 정적 랙을 하나의 `RackBody`로 구성하고 세 롤러 deck을
그 형제 articulation으로 배치한다. 장애물 센서는 `RackBody`와 3×26×7개 롤러 body를
정확한 경로로 각각 등록한다. 따라서 롤러 접촉도 `obstacle_forces`에서 빠지지 않는다.

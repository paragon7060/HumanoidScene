...
| 파지 유지 | 같은 flap 접촉은 각 0.1N 초과, 최초 파지 대비 손가락 중심 이동 2cm 이내·벌어짐 증가 8mm 이내 |
| 접촉 누락 유예 | 최초 파지 이후 0.1초 미만; 위 상대 위치·벌어짐 조건은 계속 필요 |
| 들어올리기 | 박스 본체 중심이 reset 후 안착 기준 6cm 초과 상승 |
| 기울기 | 40° 이내 |
| 안정성 | 박스 선속도 < 0.08m/s, 각속도 < 0.35rad/s |
| 유지 시간 | 위 조건을 연속 0.5초 유지 |
| 추가 접촉 제한 | 파지 손가락의 두 허용 flap 외 잔여 접촉력 < 10N |
...
기준을 절반으로 낮추되, flap 좌표계에서 손가락 두 원점의 중점이 최초 파지보다 2cm 넘게
움직이거나 손가락 간격이 8mm 넘게 증가하면 즉시 해제한다. 같은 상대 배치를 유지할 때만
0.1초 미만의 접촉 누락을 허용한다(30Hz에서 연속 2개 샘플까지). 긴 누락 뒤에는 다시
...
신호를 제공한다. 접근 reward는 `4 * exp(-12 * 거리[m])`이고 방향 정렬로 곱하지 않는다.

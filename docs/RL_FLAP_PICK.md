...
| 주변 장애물 | 모든 로봇 rigid link의 랙·펜스·버튼·컨베이어 접촉력 20N 초과 시 실패 |
| 내용물·선점 박스 | 모두 0개 |
...
접근하면 `clamp(1-d/0.10,0,1) * |닫힘축·flap법선|²`에 dt와 가중치를 곱해 더한다.
기존 reaching 값은 바꾸지 않는다. 닫힘 축 검증 방법은 [상세 가이드](RL_RIGHT_HAND_PICK.md)를 따른다.
...
각 환경은 독립적으로 대기하며, 이 기간에도 로봇과 장애물의 20N 초과 충돌 검사는 유지한다.

...
기본 입력은 `--input-mode controllers`이며 기본 매핑은 `--controller-mapping absolute`다.
...
시점은 robot head에 고정되고, VR 컨트롤러의 현재 위치가 로봇 손끝 목표에 대응한다.
기본 `--arm-orientation-weight 0.5`로 위치와 컨트롤러 방향을 함께 추종한다.
X 보정 시 손잡이와 tool의 방향 차이를 기준으로 삼으며, 0을 지정하면 방향 추종이 꺼진다.
각 검지 트리거를 절반 이상 당기면 해당 gripper가 닫히고 놓으면 열린다.
S200062의 0rad는 닫힌 자세다. 현재는 앞쪽 두 관절 -0.25rad, 뒤쪽 두 관절 +0.25rad로
약 9cm 벌려 생성·리셋한다(URDF 제한 ±0.698rad). 닫힘은 각각 -0.005/+0.005rad다.
스틱은 A/B로 따라오기를 켜고 왼쪽 아래 그립을 놓은 상태에서 작동한다.
초기 몸통 높이는 하한이므로 먼저 올린 뒤 내려올 수 있다.

기본 `--scene-detail compact`는 배경 창고 랙·팔레트·카트·적재물과 사용하지 않는
legacy totes/cargo를 제외한다. 작업용 랙·상자·컨베이어와 공장 바닥·벽·조명·재질은 유지한다.
`--scene-detail full`로 기존 배경과 비교할 수 있다.
녹화 시 빈 RGB를 쓰지 않으며, XR 카메라가 멈추지 않도록 PC용 160×90 렌더만 유지한다.
Quest 양안 해상도와 HDF5 head 640×360 해상도는 바꾸지 않는다.
...
| 왼쪽 스틱 | — | 베이스 전후/좌우 이동 |
| 오른쪽 스틱 | — | 좌우: 허리 회전, 상하: 몸통 높이 |
| 왼쪽 아래 그립 트리거 | — | 누르는 동안 자유 시점, 놓으면 robot head 복귀 |
| 양쪽 위 검지 트리거 | — | 놓으면 열림, 당기면 닫힘; 생성·R 리셋도 열림 |
...
수집용 장면에서는 사람·뒤쪽 이동 로봇을 제외하되 공장 재질·텍스처와 작업 대상은 유지한다.
로봇 기본 위치는 랙 반대 방향(-X)으로 10cm 옮겼다. 기본은 RGB만 저장하고 depth는 끈다. 별도 PC 카메라 창과 desktop 장면 렌더도 기본 OFF다.
필요하면 `--record-depth`, `--camera-preview`, `--desktop-render`로 켠다.
`--no-wrist-cameras`는 양쪽 손목 센서를 생성하지 않고 패널·저장도 끈다.
VR 해상도와 head 카메라는 유지한다. RTX 3060 대기 상태 비교는
손목 ON 15.4Hz → OFF 15.5Hz였다. Quest 외부 연결이 없는 상태여서
실제 착용·스트리밍 중 개선 폭은 별도로 검증해야 한다.
스틱은 A/B로 따라오기를 켠 상태에서 작동한다. 자유 시점에서는 새 팔·몸통 명령을 멈추고 마지막 팔 목표를 유지한다.
베이스는 **시뮬레이션용 fixed-root 평면 이동**이며 바퀴 동역학 제어가 아니다.
`--control-hz 60`은 시뮬레이션 기준 주기이며 실제 속도는 `[PERF]`로 확인한다.
VR 렌더 해상도는 원래 1.0배를 유지한다. RTX 3060의 대기 상태 측정은 약 15~16Hz(녹화 OFF)다.
Quest 연결·착용 중 실제 속도와 60Hz 달성 여부는 추가 검증이 필요하다. 해상도 축소를 기본 최적화로 사용하지 않는다.
새 기본 action은 24차원이다. 이전 데이터와 바로 혼합하지 말고
[조작·성능 안내](docs/QUEST3_KUAVO_TELEOP_GUIDE.md#4-조작-및-episode-제어)를 참고한다.
